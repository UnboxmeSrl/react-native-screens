...
- (instancetype)init
{
    self = [super init];
    if (self) {
        self.fullWidthBackGestureRecognizer = [[UIPanGestureRecognizer alloc] init];
    }
    return self;
}
...
{
    // We are assigning the same targets to our fullWidthBackGestureRecognizer that interactivePopGestureRecognizer has.
    // That way we can hijack this gesture handler with our own fullWidthBackGestureRecognizer

    if ([self interactivePopGestureRecognizer] != nil && [[self interactivePopGestureRecognizer] valueForKey:@"targets"] != nil)
    {
        UIPanGestureRecognizer *targets = [[self interactivePopGestureRecognizer] valueForKey:@"targets"];
        [[self fullWidthBackGestureRecognizer] setValue:targets forKey:@"targets"];
        [[self fullWidthBackGestureRecognizer] setDelegate:self];
        [[self view] addGestureRecognizer:[self fullWidthBackGestureRecognizer]];
    }
...
{
    RNSScreenView *topScreen = (RNSScreenView *)self.viewControllers.lastObject.view;
    if(topScreen.nativeBackGesture == true){
        return false;
    }

    BOOL isSystemSwipeToBackEnabled = [[self interactivePopGestureRecognizer] isEnabled] == true;
    BOOL isThereStackedViewControllers = [[self viewControllers ] count] > 1;
...
- (void)setModalViewControllers:(NSArray<UIViewController *> *)controllers
{
  ...
  if (presentedViewController != nil && ([_presentedModals containsObject:presentedViewController] )) {
    BOOL shouldAnimate = changeRootIndex == controllers.count && [presentedViewController isKindOfClass:[RNSScreen class]] && ((RNSScreenView *) presentedViewController.view).stackAnimation != RNSScreenStackAnimationNone;
